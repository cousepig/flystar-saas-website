...
export async function generateMetadata({ params }: any): Promise<Metadata> {
  ...
  const article = getAllShowcases(locale).find((post) => post.slug === slug);
...
export default async function PostPage({ params }: any) {
  ...
  const relatedArticle = getAllShowcases(locale).sort();
  if (!article) {
    return notFound();
  }
  ...
  const share = [
    {
      url: "https://www.syrincs.hk" + `/${locale}${slug}`,
      title: article.title,
      description: article.description,
      image: article.image,
    },
              ...
              <div className="mb-10 rounded-sm bg-white">
                ...
                <ul className="py-8">
                  {relatedArticle.map((post, index) => (
                    ...
                    >
                      <Link href={`/${locale}/showcase/${post.slug}`}>
